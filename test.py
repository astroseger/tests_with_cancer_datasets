...
def print_results_for_field(dataset, field):
    ...
    X_notrea,_     = prepare_full_dataset(dataset_notrea_dataset, y_field = field)
    ...
    rez["full_xgboost"].append(calc_results_simple(X_full, X_full, y_full, y_full, XGBClassifier()))
    rez["full_logi"].append(   calc_results_simple(X_full, X_full, y_full, y_full, LogisticRegression(max_iter=10000, C=1)))
    rez["notrea_xgboost"].append(calc_results_simple(X_notrea, X_notrea, y_full, y_full, XGBClassifier()))
    rez["notrea_logi"].append(   calc_results_simple(X_notrea, X_notrea, y_full, y_full, LogisticRegression(max_iter=1000, C=0.01)))
